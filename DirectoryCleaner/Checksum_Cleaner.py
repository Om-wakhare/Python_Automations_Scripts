###############################################################
# Project        : ChecksumCleaner - Directory Automation Tool
...
def directory_watcher(DirectoryName="TargetDir"):
    ...
    filename = f"ChecksumCleanerLog_{timestamp}.log".replace(" ", "").replace(":", "")

    with open(filename, "w") as fobj:
        Border = "-" * 54
        fobj.write(Border + "\n")
        fobj.write("This is a log file of ChecksumCleaner Automation Script\n")
...
def main():
    Border = "-" * 54
    print(Border)
    print("------------- ChecksumCleaner Automation -------------")
    print(Border)
    ...
    if len(sys.argv) == 2:
        ...
        schedule.every(int(sys.argv[2])).seconds.do(lambda: delete_duplicate(sys.argv[1]))
